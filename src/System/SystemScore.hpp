...
namespace score {
	...
	public:
		...
		const SystemScore::Header& getHeader() const noexcept;

		const std::vector<Note> &getNotes() const noexcept;

		const score::ScoreTimeConverter &getConverter() const noexcept;
		
		const Error<SystemScore::State> &getLastError() const noexcept;
	...
	private:
		...
		SystemScore::Header header;

		std::vector<Tempo> tempo;
		std::vector<Beat> beat;
		std::vector<Bar> bar;
		
		Error<SystemScore::State> prevError;
		
		void init();

	};

	using score_err_t = Error<SystemScore::State>;
