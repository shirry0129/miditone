...
namespace ui {
	...
	ButtonNote::ButtonNote(size_t _laneNum, float _wakeUpTime, float _acceleration):
	...
	ButtonHitNote::ButtonHitNote(size_t _laneNum, float _judgeTime, float _wakeUpTime, float _acceleration):
	ButtonNote(_laneNum, _wakeUpTime, _acceleration),
	judgeTime(_judgeTime){}
	
	void ButtonHitNote::update(double currentTime) {
	...
	ButtonHoldNote::ButtonHoldNote(size_t _laneNum, float _startTime, float _endTime, float _wakeUpTime, float _acceleration):
	ButtonNote(_laneNum, _wakeUpTime, _acceleration),
	startTime(_startTime),
	endTime(_endTime){}
	
	void ButtonHoldNote::update(double currentTime) {
	...
	void ButtonHoldNote::draw() const {
		if (!isJudged) {
			if(note.p3.y > 0 && note.p0.y < ::gameinfo::originalResolution.y){
